...
clubRouter.post("/api/clubs", auth, async (req, res) => {
  try {
    console.log('Received club creation request:', req.body);
    ...
    console.log('Generated club ID:', clubId);
    ...
    try {
      ...
      console.log('User document updated with new club');
    } catch (userError) {
      ...
      console.log('Created new user document with club');
    }
    
    res.status(200).json({ success: true, clubId });
  } catch (error) {
    console.error('Error creating club:', error);
    res.status(500).json({ success: false, msg: 'Failed to create club', error: error.message });
  }
});
